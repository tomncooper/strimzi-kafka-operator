    ...
    protected Future<Void> createOrUpdate(Reconciliation reconciliation, KafkaConnect kafkaConnect) {
        ...
        connectServiceAccount(namespace, connect)
                ...
                .setHandler(reconciliationResult -> {
                    ...
                    if (kafkaConnect.getMetadata().getGeneration() != null) {
                        kafkaConnectStatus.setObservedGeneration(kafkaConnect.getMetadata().getGeneration());
                    }
                    ...
                    kafkaConnectStatus.setField("todo");
                    kafkaConnectStatus.setConditions(Collections.singletonList(readyCondition));

                    updateStatus(kafkaConnect, reconciliation, kafkaConnectStatus).setHandler(statusResult -> {
                        if (statusResult.succeeded()) {
                            log.debug("Status for {} is up to date", kafkaConnect.getMetadata().getName());
                        } else {
                            log.error("Failed to set status for {}. {}", kafkaConnect.getMetadata().getName(), statusResult.cause().getMessage());
                        }
