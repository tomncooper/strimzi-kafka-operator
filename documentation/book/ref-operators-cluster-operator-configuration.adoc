...
`STRIMZI_NAMESPACE`:: A comma-separated list of {PlatformNameNamespaces} that the operator should operate in.
When not set, set to empty string, or to `*` the cluster operator will operate in all {PlatformNameNamespaces}.
