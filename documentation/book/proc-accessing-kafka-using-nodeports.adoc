...
[source,shell,subs="+quotes,attributes+"]
{cmdcli} get service _cluster-name_-kafka-external-bootstrap -o=jsonpath='{.spec.ports[0].nodePort}{"\n"}'
+
...
This can be done using `{cmdcli} get`:
[source,shell,subs="+quotes,attributes+"]
{cmdcli} get node _node-name_ -o=jsonpath='{range .status.addresses[*]}{.type}{"\t"}{.address}{"\n"}'
+
