...
ifdef::OpenShift[]
NOTE: On {OpenShiftName} this you will need to have suitable privileges to do this.
On {Minishift} you can do this by logging in with cluster administrator privileges by executing `oc login -u system:admin` before the `{cmdcli} apply` commands given above.
endif::[]
...
NOTE: For accessing the dashboard, you can use the `port-forward` command for forwarding traffic from the Grafana pod to the host. For example, you can access the Grafana UI by running `{cmdcli} port-forward grafana-1-fbl7s 3000:3000` and then pointing a browser to `http://localhost:3000`.
