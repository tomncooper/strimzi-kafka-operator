...
This guide assumes that a Kubernetes cluster is available and the
...
When no existing Kubernetes cluster is available,
ifdef::Kubernetes[`Minikube`]
ifdef::Kubernetes+OpenShift[ or ]
ifdef::OpenShift[`Minishift`]
can be used to create a local cluster.
More details can be found in xref:installing_kubernetes_and_openshift_cluster[Installing Kubernetes clusters].
endif::InstallationAppendix[]

NOTE: To run the commands in this guide, your cluster user must have the rights to manage role-based access control (RBAC).

ifdef::OpenShift[]
For more information about OpenShift and setting up OpenShift cluster, see link:https://docs.openshift.com/container-platform/3.9/welcome/index.html[OpenShift documentation].
endif::[]
